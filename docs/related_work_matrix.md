...
| LLMs Assist NLP Researchers | Critique/meta-review assistance for NLP papers | Directly studies LLM support for paper critique/meta-reviewing | RevTrack targets longitudinal concern status after revision evidence instead of generating critiques | `du-etal-2024-llms-assist` |
| GPT4 Slightly Helpful (pilot) | Pilot study of GPT-4 peer-review assistance | Empirical probe of practical LLM review assistance utility | RevTrack operationalizes this as auditable label-level transfer robustness checks | `robertson-2023-gpt4-helpful` |
| DeepReview | LLM-based paper review with staged deep-thinking pipeline and DeepReview-13K resource | Stronger recent review-generation baseline/resource | RevTrack evaluates concern-status tracking under revision, not free-form review quality | `zhu-etal-2025-deepreview` |
| DeepReviewer / ReviewRobot | Early automatic review score/comment generation systems | Foundational automatic-review generation line | RevTrack shifts from static paper-level outputs to post-revision issue-state decisions | `leng-etal-2019-deepreviewer`; `wang-etal-2020-reviewrobot` |
...
Verification note (2026-05-06): candidate references from mock pre-review were only added when a concrete DOI/title could be verified. Unverified items (for example `Paper2Rebuttal` as described in the mock review) were not added to paper citations.
