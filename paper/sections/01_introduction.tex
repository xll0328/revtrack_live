...
Many review-assistance evaluations score plausible critiques, question answering, or paper quality, but they do not test whether a system can track one concern across revision.
As a result, systems can appear competent while failing the workflow decision that matters most: which concerns remain unresolved after the evidence changes.
Our thesis is that static critique quality is a misleading proxy for longitudinal scientific judgment.
...
The empirical picture is diagnostic rather than model-chasing.
...
At the same time, stress-oriented cross-year transfer slices expose brittleness.
...
The ICLR 2023 random/stratified slice adds bounded external-validity evidence outside disagreement-focused active frontiers.
Together, these transfer slices are used as brittleness diagnostics rather than venue-wide prevalence estimates.
...
\item We provide a validated ICLR 2024 benchmark slice, a 21-row ICLR 2025 stress set, 80-row user-confirmed active frontiers for ICLR 2025 and NeurIPS 2024, and an 80-row ICLR 2023 random/stratified standard slice, and we report them with explicit stress-evidence boundaries.
