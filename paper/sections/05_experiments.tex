...
All transfer-frontier results are interpreted as stress-oriented evidence by construction, not as venue-wide prevalence estimates.
...
To address the weak-baseline concern directly, we also run a stronger learned baseline: a fine-tuned ModernBERT classifier with three random seeds (Appendix Table~\ref{tab:finetuned-modernbert-multiseed}).
In-domain performance improves over frozen semantic encoders (ICLR 2024 macro-F1 $0.531\pm0.021$), but still remains below the structured model.
Under transfer, unresolved and regressed recovery stay near zero across seeds (ICLR 2025 expanded80 unresolved F1 $0.000\pm0.000$, regressed F1 $0.000\pm0.000$), reinforcing that the main bottleneck is longitudinal transfer rather than only baseline weakness.

\subsection{RQ2: When Does Accuracy Mislead?}

\input{tables/null_baselines}
...
Appendix Table~\ref{tab:worked-issue-examples} provides worked concern/evidence examples for fixed, partially\_fixed, unresolved, and regressed boundary behavior.
For example, \texttt{w7P92BEsb2\_\_r01} is \textit{fixed} only after the revision adds direct cost/error comparisons, while \texttt{My7lkRNnL9\_\_r01} remains \textit{unresolved} because no new theory or decisive evidence is added.
...
\input{tables/expanded80_transfer}
\input{tables/neurips_limit100_transfer}

\textbf{In-domain to ICLR 2025} is still the strongest signal for in-year revision-aware gain, but both expanded80 and NeurIPS limit100 are purposefully hard transfer frontiers.
Across venues, the same pattern holds: high aggregate scores can still hide clinically important misses, especially on unresolved concerns.
...
Appendix Table~\ref{tab:split-label-coverage} reports the exact split-level label coverage and missing-label patterns used for boundary interpretation.
...
This supports a cross-year brittleness claim, but not a broad prevalence claim, because these frontiers are disagreement-harvested hard subsets.
...
\input{tables/prompted_llm_ensemble}
...
\input{tables/prompted_llm_temperature_ablation}
