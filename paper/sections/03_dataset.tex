...
First, we collect public OpenReview submissions and discussion notes from the platform~\citep{openreview}.
...
Separately, two bounded second-pass reliability packets are tracked.
An independent mini60 packet is complete with agreement $=1.0$, Cohen's $\kappa=1.0$, and zero mismatches.
A broader boundary160 packet is also complete after explicit user confirmation of every row, again with agreement $=1.0$, Cohen's $\kappa=1.0$, and zero mismatches.
Because boundary160 used prelabel assistance before user confirmation, we treat it as user-confirmed second-pass coverage rather than blind-independent IAA.
Together, mini60 and boundary160 provide boundary-focused protocol-consistency evidence on hard cases rather than full-scale two-annotator coverage of all packets.
