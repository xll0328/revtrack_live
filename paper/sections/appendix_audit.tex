...
Table~\ref{tab:finetuned-modernbert-probe} reports a single-run fine-tuned ModernBERT classifier probe trained on ICLR2024 train v8.
This addresses the concern that frozen encoder baselines may be too weak: the probe improves in-domain performance but still collapses unresolved recovery on transfer splits.
\input{tables/finetuned_modernbert_probe}
