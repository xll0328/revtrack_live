...
\paragraph{Fine-tuned stronger-baseline probe.}
Table~\ref{tab:finetuned-modernbert-multiseed} reports a multi-seed fine-tuned ModernBERT classifier probe trained on ICLR2024 train v8.
This addresses the concern that frozen encoder baselines may be too weak: in-domain performance improves, but unresolved/regressed transfer recovery remains near-zero across seeds on frontier splits.
\input{tables/finetuned_modernbert_multiseed_probe}

\paragraph{Oral/rebuttal evidence panel.}
Table~\ref{tab:oral-evidence-panel} summarizes the highest-signal checkpoints used in oral/rebuttal discussion: in-domain gain, accuracy-trap evidence, transfer-frontier brittleness, prompted-significance boundary, bounded second-pass reliability, and readiness-gate status.
...
\paragraph{Split-level label coverage panel.}
Table~\ref{tab:split-label-coverage} makes missing-label coverage explicit for each paper-facing split.
It is used to keep four-way interpretation bounded when a split omits one or more labels (for example, NeurIPS 2024 limit100 has no fixed or regressed rows in the current standard packet).
\input{tables/split_label_coverage}
...
For a broader boundary160 packet, user-confirmed second-pass labels are complete (160/160), also with agreement $=1.0$, Cohen's $\kappa=1.0$, and zero mismatches.
Because boundary160 was prelabel-assisted before confirmation, we treat it as user-confirmed bounded reliability evidence rather than blind-independent IAA.
These artifacts are tracked in \path{iaa_second_annotator_mini60_v1_metrics.json} and \path{iaa_second_annotator_boundary160_v1_metrics.json}, and surfaced by the readiness gate as bounded reliability evidence rather than full-scale two-annotator coverage.
